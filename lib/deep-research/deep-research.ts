...
export async function writeFinalReport({
  ...
  const res = await generateObject({
    model,
    system: systemPrompt(),
    prompt: `Given the following prompt from the user, write a final report on the topic using the learnings from research and format it in proper LaTeX. Use LaTeX syntax (headings, formulas, lists, horizontal rules, etc.) to structure the document. Aim for a detailed report of Maximum 1 page. \n\n<prompt>${prompt}</prompt>\n\n`,
    schema: z.object({
      reportMarkdown: z
        .string()
